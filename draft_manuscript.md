...
# The Collegiate Network Landscape

## Aggregate Evolution and Functional Decomposition

To resolve the substantive meaning of personal network evolution across college, we first examine the empirical trajectory of ego network size across all eight collegiate waves, extending from matriculation in August 2015 through senior graduation in May 2019 (*N*=457). Table 1 displays the empirical means and standard errors for Total Degree, Close Ties, Daily Activated Ties, and Support-Providing Ties across all eight semesters. Because the social support battery was first introduced into the NetHealth survey instrument during Wave 2 (freshman spring) and temporarily rotated out during Wave 6 (junior spring) due to survey modularization, support-providing degree cannot be computed for Waves 1 or 6; hence, these cells are designated with a dash in Table 1.
...
*Note:* Standard errors in parentheses. Support questions were not administered in Wave 1 (freshman fall) or Wave 6 (junior spring); dashes indicate waves where the support battery was omitted.
...
Interpreted through Granovetter's (1973) classic distinction between weak and strong ties, this functional decomposition reveals that the apparent downward degree trajectory widely documented in aggregate student surveys does not signify social isolation or an erosion of core supportive communities. Rather, students maintain a highly resilient, temporally stable inner core of strong ties, comprised of daily activated associates (matching Dunbar's support clique of roughly 5 alters) and close, supportive ties (matching the sympathy group of roughly 10 to 12 alters). The aggregate decline in network size is driven almost entirely by the winnowing of weak ties that were temporarily accumulated during the initial disorganization of freshman year. Once students establish enduring campus niches, they prune weak ties to conserve cognitive and temporal bandwidth, while insulating their strong ties from decay.

## Partitioning Social Support: Functional Specificity and Multiplexity

While the aggregate supportive subnetwork in Table 1 demonstrates general stability, treating any alter who provides at least one form of assistance as an undifferentiated supportive tie conceals meaningful variation in relational depth. Interpersonal support exchanges encompass qualitatively distinct functional domains, ranging from casual companionship to deep emotional vulnerability and material resource pooling (Marsden & Campbell, 1984; Wellman & Wortley, 1990). To examine whether personal networks can be partitioned further by support type and multiplexity, Table 2 decomposes the supportive subnetwork into four specific functional categories---companionship (hanging out), advice (academic and personal guidance), emotional comfort (comfort when upset), and financial assistance---alongside a structural partition by support multiplexity (uniplex ties providing exactly one support function versus multiplex ties providing two or more distinct supports).

| Academic Wave | Egos (N) | Hang Out (SE) | Advice (SE) | Comfort (SE) | Finance (SE) | Uniplex (SE) | Multiplex (SE) |
|:---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
...
Table 3 reports the information-theoretic fit statistics across candidate models.
...
The three latent classes capture distinct developmental archetypes of personal network evolution across the undergraduate career. First, **Network Conservers** (*n*=131, 28.7%) represent students who matriculate with large personal networks (*D*‾<sub>1</sub>=17.60) and preserve an exceptionally stable network scale throughout their entire undergraduate careers (*D*‾<sub>8</sub>=17.43), exhibiting essentially zero net winnowing over four years. Second, **Moderate Winnowers** (*n*=182, 39.8%) define the modal collegiate pathway, representing students who matriculate with average-sized circles (*D*‾<sub>1</sub>=14.46) and experience a steady, progressive winnowing across semesters, shedding roughly five to six alters by graduation (*D*‾<sub>8</sub>=9.03). Third, **Accelerated Winnowers** (*n*=144, 31.5%) enter college with somewhat smaller networks (*D*‾<sub>1</sub>=10.95), undergo an aggressive contraction across freshman and sophomore years, and stabilize at an intimate core of roughly 4 to 5 alters through senior graduation (*D*‾<sub>8</sub>=4.78).
...
Table 4 reports the multinomial logit parameter estimates, standard errors, Odds Ratios (Relative Risk Ratios), 95% confidence intervals, and *p*-values for both Network Conservers and Accelerated Winnowers relative to Moderate Winnowers.
...
The multinomial regression results provide clear empirical evaluation of our core hypotheses. First, consistent with **Hypothesis 1**, classic indicators of family socioeconomic background—including parental income (OR=1.04, *p*=0.601 for Conservers; OR=0.99, *p*=0.887 for Winnowers) and maternal college degree (OR=0.93, *p*=0.871; OR=1.00, *p*=0.993)—exhibit zero predictive power. In contrast, psychological traits and trust display substantial substantive associations.

Second, supporting **Hypothesis 2**, higher baseline Generalized Trust increases the odds of belonging to the Network Conserver class by 31% per standard deviation (OR=1.31, 95% CI: [0.98, 1.76], *p*=0.072), while reducing the likelihood of belonging to the Accelerated Winnower class (OR=0.87). Trusting students are systematically more likely to sustain large, durable networks across all four years.

Finally, with respect to racial background and institutional context, students identifying as Other or International face significantly higher odds of belonging to the Accelerated Winnower class (OR=3.02, 95% CI: [1.07, 8.51], *p*=0.037), reflecting institutional friction and social distance on a predominantly white residential campus.
