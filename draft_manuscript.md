...
Figure 8 displays boxplots of likelihood ratio *χ*<sup>2</sup> *p*-values across model families. In support of **Hypothesis 1**, personality models consistently outperform demographic models. While the majority of demographic models fail to achieve omnibus statistical significance (median *p*=0.254), personality models systematically yield significant likelihood ratio tests (median *p*=0.018), with over 72% falling below the conventional *p*=0.05 threshold. Mixed models display intermediate fit statistics (median *p*=0.034).

<img src="media/image5.png" style="width:6.5in;height:4.64286in" />

Figure 9 presents the distribution of McFadden pseudo-*R*<sup>2</sup> values across model families. Overall explanatory power is modest, ranging between 0.02 and 0.09, replicating the benchmark findings of Chandler and Hachen (2018). While demographic specifications yield a slightly higher raw pseudo-*R*<sup>2</sup> due to the inclusion of multi-category categorical factors (such as five-category race and eight-category income), Figure 10 demonstrates that when partitioning models by omnibus significance, statistically significant models in all three families converge to an average pseudo-*R*<sup>2</sup> of roughly 0.04 to 0.06.
...
The three latent classes capture distinct relational strategies across college: 1. **Class 1: Network Conservers** (*n*=208,46.2%): Trajectories hover stably around a moderate degree (≈12−14 alters) throughout all six waves, exhibiting little net change over time. 2. **Class 2: Accelerated Winnowers** (*n*=162,36.0%): Students who matriculate with exceptionally large networks (*D*‾<sub>1</sub>≈18.5) but experience an aggressive, continuous contraction, shedding an average of 8 alters by junior year (*D*‾<sub>6</sub>≈10.2). 3. **Class 3: Tie Accumulators** (*n*=80,17.8%): Students who enter college with smaller networks (*D*‾<sub>1</sub>≈7.5) and steadily expand their social circles across sophomore and junior years, reaching an average of 14.5 alters.
...
Figure 6 illustrates this dynamic by plotting predicted degree trajectories across combinations of high versus low Extraversion (±1 SD) and high versus low Neuroticism (±1 SD). As shown in the figure, highly extraverted students matriculate with considerably larger networks (≈15.5 alters) compared to introverted peers (≈12.0 alters). However, extraverts undergo aggressive winnowing across college, converging toward a common collegiate equilibrium (≈10−11 alters) by junior year. Introverted students, having formed more selective connections initially, experience far more gradual declines.
...
The empirical pattern in Figure 7 provides decisive support for **Hypothesis 4** and fundamentally transforms the interpretation of collegiate network evolution: - **Total Ego Network Size** (blue line) exhibits a steady downward decline, falling from 14.19 alters in freshman fall (Wave 1) to 10.64 in junior spring (Wave 6) and stabilizing at 10.80 in senior spring (Wave 8). - In sharp contrast, **Daily Activated Ties** (red line) remain remarkably flat across all four years, hovering consistently between 4.4 and 5.9 ties per wave. - Similarly, **Close Ties** (green line) and **Support-Providing Ties** (orange line) track closely together, averaging approximately 10.5 to 12.0 ties across college.
