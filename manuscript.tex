...
Figure~\ref{fig:fig8} displays boxplots of likelihood ratio $\chi^2$ $p$-values across model families. In support of Hypothesis 1, personality models consistently outperform demographic models. While the majority of demographic models fail to achieve omnibus statistical significance (median $p = 0.254$), personality models systematically yield significant likelihood ratio tests (median $p = 0.018$), with over 72\% falling below the conventional $p = 0.05$ threshold. Mixed models display intermediate fit statistics (median $p = 0.034$).
...
Figure~\ref{fig:fig9} presents the distribution of McFadden pseudo-$R^2$ values across model families. Overall explanatory power is modest, ranging from 0.02 to 0.09  \citep{chandler2018classifying}. While demographic specifications yield a slightly higher raw pseudo-$R^2$ due to the inclusion of multi-category factors (such as five-category race and eight-category income), Figure~\ref{fig:fig10} shows that when partitioning models by omnibus significance, statistically significant models in all three families converge to an average pseudo-$R^2$ of roughly 0.04 to 0.06.
